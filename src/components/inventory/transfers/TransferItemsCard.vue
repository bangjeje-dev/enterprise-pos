      ...
      <table class="w-full text-sm text-left text-gray-500">
        <thead class="text-xs text-gray-700 uppercase bg-gray-50 border-b border-gray-200">
          <tr>
            ...
            <th v-if="isReceiving" scope="col" class="px-5 py-3 w-40 text-right">Receive Now</th>
        ...
        <tbody class="divide-y divide-gray-200">
          <tr v-for="(item, index) in items" :key="item.id" class="bg-white">
            ...
            <td v-if="isReceiving" class="px-5 py-4 text-right">
              <input 
                type="number" 
                v-model="item.draftReceiveQty"
                min="0"
                :max="item.transferQty - item.receivedQty"
                class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2 text-right"
                placeholder="Qty"
              >
